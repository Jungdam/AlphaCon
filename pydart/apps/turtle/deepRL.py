...
class DeepRLBase:
	...
	def determine_exploration(self):
		return np.random.uniform(0.0,1.0) < self.exprolation_prob
...
class DeepRLSimple(DeepRLBase):
	def __init__(self, world, scene, nn, warmup_file=None):
		DeepRLBase.__init__(self, warmup_file)
		self.replay_buffer['actor'] = ReplayBuffer()
		self.replay_buffer['critic'] = ReplayBuffer()
		self.world = world
		self.scene = scene
		self.nn = nn
		if warmup_file is not None:
			...
			num_train_data = 2*self.replay_buffer['actor'].size
			while True:
